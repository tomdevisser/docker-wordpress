...
sshpass -p "$remote_pass" ssh -p "$remote_port" -o StrictHostKeyChecking=no "$remote_user@$remote_host" "cd $remote_path && wp db export --allow-root $remote_sql_file > /dev/null 2>&1"
if [[ $? -ne 0 ]]; then
...
remote_siteurl=$(sshpass -p "$remote_pass" ssh -p "$remote_port" -o StrictHostKeyChecking=no "$remote_user@$remote_host" "cd $remote_path && wp option get siteurl --allow-root" 2>/dev/null)
...
sshpass -p "$remote_pass" scp -P "$remote_port" -o StrictHostKeyChecking=no "$remote_user@$remote_host:$remote_path/$remote_sql_file" "$local_sql_file" > /dev/null 2>&1
if [[ $? -ne 0 ]]; then
...
sshpass -p "$remote_pass" ssh -p "$remote_port" -o StrictHostKeyChecking=no "$remote_user@$remote_host" "rm $remote_path/$remote_sql_file" > /dev/null 2>&1

# Import into local WordPress
# Copy SQL file into the container
container_path="/var/www/html/$remote_sql_file"
docker cp "$local_sql_file" dev-wordpress:"$container_path" > /dev/null 2>&1

echo "Importing external database into local WordPress..."

docker exec dev-wordpress wp db import "$container_path" --allow-root > /dev/null 2>&1
if [[ $? -ne 0 ]]; then
...
docker exec dev-wordpress wp search-replace "$remote_siteurl" "https://$domain" --all-tables --allow-root > /dev/null 2>&1

# Remove the SQL file from the container after import (cleanup)
docker exec dev-wordpress rm "$container_path" > /dev/null 2>&1
